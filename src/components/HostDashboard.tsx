...
export function HostDashboard({
  ...
  const { state, viewerCount, isSharing, startSharing, stopSharing } = host;

  const statusText =
    state === "initializing" ? "Opening frequency..."
      : state === "waiting" ? `Waiting for a device to tune in on ${roomCode}.`
      : state === "connected" ? `${viewerCount} ${viewerCount === 1 ? "viewer" : "viewers"} standing by.`
      : state === "live" ? `Live to ${viewerCount} ${viewerCount === 1 ? "viewer" : "viewers"}.`
      : "";
  ...
  return (
    <div className="grid grid-cols-1 gap-0 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <section className="flex flex-col gap-8 px-6 py-10 lg:px-12 lg:py-14">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.35em] text-signal">Your frequency</p>
        ...
        <div className="border border-panel-line bg-panel p-6">
          ...
          <div className="mt-4"><Waveform state={waveState} bars={48} /></div>
        ...
        <div className="flex flex-col gap-3">
          {!isSharing ? (
            ...
            >
              <div>
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] opacity-70">Transmit</div>
            ...
            >
              <div>
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] opacity-80">Cut signal</div>
